...
/* Немного теории.
   ...
   же типа, что и массив. Кол-во элементов обычного массива должно быть известно
   на этапе компиляции.
*/
int main()
{
  //тип данных в массиве может быть абсолютно любым
    ...
    //объявлен тип: integer; имя: arr; кол-во элементов: 3(это число должно быть константой); 1й элемент=3, 2й= 7, 3й=5
    int arr [ 3 ] = { 3, 7, 5 };
...
/*Выделение динамической памяти. Указав тип переменной и ее имя со свездой
  пишем ключевое слово "new"  и тип данных, с которыми мы работаем.
*/
  int *ptrnum = new int(7);//теперь мы динамически выделили память под объект типа "int" и присвоили начальное значение(7)
  //так же, из-за того что выделена динамическая память, можно менять значения.
  *ptrnum = 7;

  cout << *ptrnum<< endl;
//после того, как все действия с динамическим объектом выполнены необходимо очистить после него память
//для этого есть ключевое слово "delete". после него прописываем имя переменной, но без звездочки(т.к мы удаляем саму переменную, вместе с адресом)
  delete ptrnum;



/*Динамические массивы- массив с изменяемым кол-вом элементов.
"*ptrarray"- указатель на выделенный под массив участок памяти, а в "[]" мы
указали начальное кол-во элементов в массиве
*/
...
/*Так же можно указать и кол-во элементов позже*/
int amount =0;
int *ptr= NULL;
...
{
  ptr[i] = rand %100;
  cout<<ptr[i]<<"\t";// вывели массив и в конце табуляцию
}
